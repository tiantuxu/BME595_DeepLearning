...
class NeuralNetwork:
    def __init__(self, layers):
        ...
        for i in range(self.num_layers - 1):
            if i == 0:
                self.Theta[self.key[i]] = torch.normal(mean=torch.zeros(self.input_layer + 1, self.layers[i+1]), std=torch.zeros(self.input_layer + 1, self.layers[i+1]).fill_(1.0/math.sqrt(self.input_layer + 1))).type(torch.FloatTensor)
            else:
    ...
    def forward(self, input):
        ...
        for i in range(0, self.num_layers - 1):
            ...
            self.z[i + 1] = torch.mm(self.Theta[self.key[i]].t(), self.a[i])
            self.a[i + 1] = 1.0 / (1.0 + np.exp(-self.z[i + 1]))

        return self.a[self.num_layers - 1].t()
